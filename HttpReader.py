...
class HttpReader:
	...
	def __init__(self, sock, connection, addr):
		self.sock = sock
	...
	def start(self):
		log.info("starting read from " + str(self.addr))
		while(not self.end):
			message = None
			...
			try:
				...
				if(len(message) == 0):
					log.info("received empty message from " + str(self.addr))
			...
			if not self.end:
				...
				if not self.connection.isTunnel:
					if HttpParser.isConnect(message):
						self.connection.isTunnel = True
					else:
						message = HttpParser.modifyMessage(message)
				self.converter.putHttp((self.DATA_CMD, (self.addr, message)))
		if(self.connection.writer.end):
			self.sock.close()
